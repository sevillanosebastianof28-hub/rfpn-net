...
const navItems = [
  ...
  { icon: User, label: 'My Profile', path: '/developer/profile' },
...
export default function DeveloperLayout() {
  ...
  if (user?.role !== 'developer') return <Navigate to="/login" replace />;
    ...
    <div className="min-h-screen bg-background">
      <aside className={cn('fixed left-0 top-0 z-40 flex h-screen flex-col border-r border-sidebar-border bg-sidebar transition-all duration-300', collapsed ? 'w-[72px]' : 'w-64')}>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto p-3">
          {!collapsed && <p className="mb-2 px-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Developer Portal</p>}
        ...
        {collapsed && (
          <Button variant="ghost" size="icon" onClick={() => setCollapsed(false)}
            className="absolute -right-3 top-20 h-6 w-6 rounded-full border border-sidebar-border bg-sidebar text-muted-foreground">
